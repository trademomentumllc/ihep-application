...
const loadThreeDependencies = async () => {
    ...
    const [threeModule, controlsModule] = await Promise.all([
        import('three'),
        import('../../lib/three/OrbitControls.js'),
    ]);
...
class IHEPDigitalTwinRenderer {
    ...
    private loadedUSDModel: { clean: () => void } | null = null;
    private humanoidGroup: THREE.Group | null = null;
    ...
    constructor(containerElement: HTMLElement, options: RendererOptions = {}) {
        ...
        // Core three.js components are initialized in initialize()
    ...
    addReferenceGeometry() {
        ...
        // Add a visible 3D humanoid figure as default visualization
        this.addHumanoidFigure();
    ...
    /**
     * Create a 3D humanoid figure for health visualization.
     * This provides a visible model when no USD scene is loaded.
     */
    addHumanoidFigure() {
        const humanoid = new THREE.Group();
        humanoid.name = 'HumanoidFigure';

        // Material with health-indicating color (green = healthy)
        ...
        // Torso
        const torsoGeometry = new THREE.CapsuleGeometry(0.4, 1.2, 16, 32);
        ...
        // Arms
        const armGeometry = new THREE.CapsuleGeometry(0.12, 0.8, 16, 16);
        ...
        // Legs
        const legGeometry = new THREE.CapsuleGeometry(0.15, 1.0, 16, 16);
        const legMaterial = bodyMaterial.clone();
        legMaterial.opacity = 0.5;
        ...
        // Heart indicator (pulsing sphere)
        ...
        heart.position.set(-0.1, 1.0, 0.2);
        heart.name = 'HeartIndicator';
        humanoid.add(heart);

        // Store reference for animation
        this.humanoidGroup = humanoid;

        this.scene.add(humanoid);
    }
     ...
     */
    async loadUSDScene(usdPath: string): Promise<void> {
        await this.ready;
        if (this.disposed) return;
        // USDZ loader temporarily disabled - using built-in humanoid visualization
        console.warn('USDZ loading is temporarily disabled. Using built-in humanoid figure.');
        console.log(`Requested USD scene: ${usdPath}`);
        // The renderer already creates a humanoid figure in addHumanoidFigure()
     ...
     */
    instantiatePatientTwin(usdNode: THREE.Object3D) {
     ...
     */
    extractTrajectoryPoints(curveNode: THREE.Object3D): THREE.Vector3[] {
        ...
        const meshNode = curveNode as THREE.Mesh;
        if (meshNode.geometry && meshNode.geometry.attributes.position) {
            const positions = meshNode.geometry.attributes.position.array;
     ...
     */
    extractColorFromUSD(usdNode: THREE.Object3D): THREE.Color {
        // Check for displayColor attribute in material
        const meshNode = usdNode as THREE.Mesh<THREE.BufferGeometry, THREE.MeshStandardMaterial>;
        if (meshNode.material && meshNode.material.color) {
            return new THREE.Color(
                meshNode.material.color.r,
                meshNode.material.color.g,
                meshNode.material.color.b
            );
        }
     ...
     */
    extractIndicatorData(indicatorNode: THREE.Object3D): IndicatorData {
    ...
    animate() {
        ...
        // Animate humanoid figure (slow rotation + heart pulse)
        if (this.humanoidGroup) {
            this.humanoidGroup.rotation.y += 0.005;

            // Pulse heart indicator
            const heart = this.humanoidGroup.getObjectByName('HeartIndicator');
            if (heart) {
                const pulse = 1 + 0.1 * Math.sin(frameStartTime * 0.005);
    ...
    dispose() {
        ...
        // Clean up loaded USD model
        if (this.loadedUSDModel) {
            this.loadedUSDModel.clean();
            this.loadedUSDModel = null;
        }
