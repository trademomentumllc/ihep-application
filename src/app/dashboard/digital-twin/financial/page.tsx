...
async function getSnapshot(): Promise<TwinSnapshot> {
  const res = await fetch('/api/twins/financial', { cache: 'no-store' });
  if (!res.ok) {
    throw new Error('Failed to load financial twin data');
  }
  return res.json();
}
  ...
  return (
    <section className="space-y-4">
      <header>
        ...
        <h1 className="text-2xl font-semibold">Financial Twin</h1>
