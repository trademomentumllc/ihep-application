...
The project is in late development (v1.1.0-alpha) with a polished landing page and prepared infrastructure, located in an external folder for backend development. 
This is an enterprise application and is not a simulation. You are to approach this project as you would with users privacy and PHI as a central concern. You are not to use Emojis in any output. NO --pretty:anything! Everything that is implemented must verify and prove mathematically with trusted formulas, otherwise DO NOT USE IT! This is an Human/AI Joint Venture Application where 5 humans will oversee managed agentic clusters of 25 agents(1 human/5 agents). Each cluster will also be assigned an overseer agent. Each of these overseers will be managed by a supervisory position within the institution that oversees the teams. On each start refer to the SESSION_HANDOFF.md, the TODO.md, and the PROJECT_SUMMARY.md for any updates and to gain a perspective of what to do next without wasting context. When you get to a 5% context remaining, immediately transfer the session by writing to the SESSION_HANDOFF and TODO files for the next agent.
...
- **Database:** Prisma (ORM installed but no schema configured)
- **Auth:** JWT + bcryptjs (installed but not integrated)
...
- `app/api/` - API route handlers (structure prepared, implementations pending)
  - `auth/` - Authentication endpoints
  - `appointments/` - Appointment management
  - `wellness/` - Wellness tracking
  - `resources/` - Resource hub
- `src/` - Source files with path alias `@/*`

### Current State
- Landing page (`app/page.tsx`) is fully functional with client-side React
- API routes have directory structure but no implementations
- Prisma installed without schema configuration
- Authentication libraries installed but not integrated
...
### Styling
- Purple-pink gradient color scheme (`from-purple-600 to-pink-600`)
