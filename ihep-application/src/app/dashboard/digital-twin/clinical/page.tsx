...
async function getSnapshot(): Promise<TwinSnapshot> {
  const res = await fetch('/api/twins/clinical', { cache: 'no-store' });
  if (!res.ok) {
    throw new Error('Failed to load clinical twin data');
  }
  return res.json();
}

export default async function ClinicalTwinPage() {
  const snapshot = await getSnapshot();

  return (
    <section className="space-y-4">
      <header>
        <p className="text-sm text-muted-foreground">Clinical twin</p>
        <h1 className="text-2xl font-semibold">Clinical Twin</h1>
