...
async function getSnapshot(): Promise<TwinSnapshot> {
  const res = await fetch('/api/twins/social', { cache: 'no-store' });
  if (!res.ok) {
    throw new Error('Failed to load social twin data');
  }
  return res.json();
}

export default async function SocialTwinPage() {
  const snapshot = await getSnapshot();

  return (
    <section className="space-y-4">
      <header>
        <p className="text-sm text-muted-foreground">Social twin</p>
        <h1 className="text-2xl font-semibold">Social Twin</h1>
