...
async function getSnapshot(): Promise<TwinSnapshot> {
  const res = await fetch('/api/twins/personal', { cache: 'no-store' });
  if (!res.ok) {
    throw new Error('Failed to load personal twin data');
  }
  return res.json();
}

export default async function PersonalTwinPage() {
  const snapshot = await getSnapshot();

  return (
    <section className="space-y-4">
      <header>
        <p className="text-sm text-muted-foreground">Personal twin</p>
        <h1 className="text-2xl font-semibold">Personal Twin</h1>
